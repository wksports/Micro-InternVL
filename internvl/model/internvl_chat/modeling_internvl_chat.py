...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(self, config: InternVLChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        # Micro-InternVL: optional high-resolution patch detection head
        self.micro_internvl_config = getattr(config, 'micro_internvl_config', None)
        if self.micro_internvl_config is not None:
            ...
            logger.info('Micro-InternVL detection head initialized')
        else:
            self.detection_head = None
            self.text_embeddings = None
        self._micro_internvl_tokenizer = None
    ...
    def extract_feature(self, pixel_values, return_uncompressed=False):
        ...
        raw_patches = vit_embeds[:, 1:, :].contiguous()

        h = w = int(raw_patches.shape[1] ** 0.5)
        vit_embeds = raw_patches.reshape(vit_embeds.shape[0], h, w, -1)
        vit_embeds = self.pixel_shuffle(vit_embeds, scale_factor=self.downsample_ratio)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], -1, vit_embeds.shape[-1])
        vit_embeds = self.mlp1(vit_embeds)
        if return_uncompressed:
            return vit_embeds, raw_patches
        return vit_embeds

    def encode_text_queries(self, queries):
        """Encode text queries using the frozen LLM input embeddings.

        Args:
            queries: List of text strings.

        Returns:
            text_embeds: [len(queries), hidden_size], L2-normalized.
        """
        from transformers import AutoTokenizer
        # tokenizer should be passed in or inferred; here we use the language_model's tokenizer
        # This is a helper: in practice the training script will pass its tokenizer.
        if not hasattr(self, '_micro_internvl_tokenizer') or self._micro_internvl_tokenizer is None:
            raise RuntimeError(
                'Tokenizer not set for Micro-InternVL text encoding. '
                'Call model.set_micro_internvl_tokenizer(tokenizer) first.'
            )
        tokenizer = self._micro_internvl_tokenizer
        ...
        mask = inputs['attention_mask'].unsqueeze(-1).to(token_embeds.dtype)
        pooled = (token_embeds * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1.0)
        pooled = pooled / (pooled.norm(dim=-1, keepdim=True) + 1e-8)
        return pooled

    def set_micro_internvl_tokenizer(self, tokenizer):
        """Set tokenizer used by encode_text_queries."""
        self._micro_internvl_tokenizer = tokenizer

    def set_text_embeddings(self, text_embeds):
        """Cache pre-computed text embeddings for open-vocabulary inference."""
        self.text_embeddings = text_embeds

    def forward_detection(self, pixel_values, text_embeddings=None, return_patch_features=False):
        """Micro-InternVL detection forward.
        ...
        if self.detection_head is None:
            raise RuntimeError('Micro-InternVL detection head is not initialized. '
                               'Set micro_internvl_config in the model config.')
        ...
        outputs = {
            ...
            'pred_objectness': head_out['pred_objectness'],
        }
        ...
        if text_embeds is not None:
            ...
            outputs['pred_logits'] = logits / temperature
